/**
 * Email Notifications System for Get Wallet
...
function generateEmailTemplate(
  ...
  const templates: Record<EmailTemplateType, (d: Record<string, any>) => { subject: string; body: string }> = {
    deposit_approved: (d) => ({
      subject: 'تم قبول إيداعك - Get Wallet',
      body: `
مرحباً ${d.userName}،
...
فريق Get Wallet
    ...
    withdraw_processed: (d) => ({
      subject: 'تم معالجة سحبك - Get Wallet',
      body: `
مرحباً ${d.userName}،
...
فريق Get Wallet
    ...
    transfer_received: (d) => ({
      subject: 'لقد استلمت تحويلاً - Get Wallet',
      body: `
مرحباً ${d.userName}،
...
فريق Get Wallet
    ...
    order_completed: (d) => ({
      subject: 'تم تنفيذ طلبك - Get Wallet',
      body: `
مرحباً ${d.userName}،
...
فريق Get Wallet
    ...
    kyc_verified: (d) => ({
      subject: 'تم توثيق حسابك - Get Wallet',
      body: `
مرحباً ${d.userName}،
...
فريق Get Wallet
    ...
    security_alert: (d) => ({
      subject: '⚠️ تنبيه أمني - Get Wallet',
      body: `
مرحباً ${d.userName}،
